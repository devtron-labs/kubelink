...
func NewFluxApplicationServiceImpl(logger *zap.SugaredLogger,
	clusterRepository clusterRepository.ClusterRepository,
	k8sUtil k8sUtils.K8sService,
	converter converter.ClusterBeanConverter, helmAppService service.HelmAppService) *FluxApplicationServiceImpl {
...
func (impl *FluxApplicationServiceImpl) BuildFluxAppDetail(request FluxAppDetailRequest) ([]*bean.ResourceTreeResponse, error) {
	...
	if request.IsKustomize {
		...
		if len(fluxHrList) > 0 {
			for _, fluxHr := range fluxHrList {
				...
				helmRelease, err := impl.getHelmRelease(request.Config, namespace, releaseName)
				if err != nil {
					return fluxAppTreeResponse, err
				}
				...
				resourceTreeResponse, err := impl.helmAppService.BuildResourceTree(appDetailRequest, helmRelease)
				if err != nil {
					return fluxAppTreeResponse, err
		...
		if len(fluxK8sResourceList) > 0 {
			...
			resourceTreeResponse, err := impl.helmAppService.GetResourceTreeForExternalResources(req)
			if err != nil {
				return fluxAppTreeResponse, err
			}
...
func (impl *FluxApplicationServiceImpl) getHelmRelease(clusterConfig *client.ClusterConfig, namespace string, releaseName string) (*release.Release, error) {
...
func (impl *FluxApplicationServiceImpl) getFluxResourceAndFluxHrList(app *FluxAppDetailRequest, fluxK8sResourceList []*client.ExternalResourceDetail, fluxHrList []*FluxHr) error {
	...
	resp, err := impl.k8sUtil.GetResource(context.Background(), app.Namespace, app.Name, GvkForKustomizationFluxApp, restConfig)
	if err != nil {
	...
	for id, version := range inventoryMap {
		var fluxResource FluxKsAppDetail
...
func parseObjMetadata(s string) (FluxKsAppDetail, error) {
